...
hw2 <- replace(hw2, c(4,12,22,27), NA) #replaces the hw2 distribution, adds NAs at the position numbers given by c numbers
hw2 #view hw2 object
...
hw2 <- na.omit(hw2) #remove nas (they don't behave! is.na() and complete.cases() also useful)
prob1 <- hw2[hw2 >= 14 & hw2 <= 38] # returns values from hw2 between 14 and 38 inclusive
prob1 <- hw[!is.na(hw2)] #alternate solution from class
